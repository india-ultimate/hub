...
import { calculateLatePenalty } from "../../utils";
    ...
    <div class="w-full rounded-lg p-2">
      <Show when={componentProps.playerFee > 0}>
        {(() => {
          ...
          const { daysLate, penalty: perPlayerPenalty } = calculateLatePenalty(
            event?.player_registration_end_date,
            event?.player_late_penalty,
            event?.player_late_penalty_end_date
          );
          const feePerPlayer = componentProps.playerFee + perPlayerPenalty;
          const formatFee = amount => `₹${(amount / 100).toLocaleString()}`;
          ...
          return (
            <div class="mb-4 rounded-lg bg-blue-50 p-4 text-sm text-blue-800 dark:bg-gray-800 dark:text-blue-400">
              ...
              <Show when={daysLate > 0}>
                <p class="mt-1 text-yellow-700 dark:text-yellow-400">
                  Late penalty: {formatFee(perPlayerPenalty)}/player ({daysLate}{" "}
                  day{daysLate > 1 ? "s" : ""} x{" "}
              ...
              >
                <h2 class="mt-1">
                  Paying{" "}
                  <strong>
                    {formatFee(feePerPlayer * selectedPlayers().length)}
                  </strong>{" "}
                  for {selectedPlayers().length} player
                  {selectedPlayers().length > 1 ? "s" : ""}
                  <Show when={daysLate > 0}>
                    {" "}
                    ({formatFee(componentProps.playerFee)}/player +{" "}
                    {formatFee(perPlayerPenalty)} penalty)
                  </Show>
                </h2>
