...
class TestRequestingAMerge(MergeFlowTestCase):
    ...
    def test_the_page_does_not_show_where_an_old_address_lives_now(self) -> None:
        ...
        they never typed. Full addresses are for a group detection built
        (spec §4); here the requester would be reading back a stranger's
        current inbox from an old one they happened to know."""
        EmailAlias.objects.create(email="gone@x.com", user=self.old)
