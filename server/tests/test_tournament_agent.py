...
class TournamentAgentToolTests(TestCase):
    ...
    def test_ask_user_honours_allow_other(self) -> None:
        """A strict multiple choice has to be able to refuse free text."""
        with self.assertRaises(AskUserPause) as cm:
            ask_user(
                self.ctx,
                prompt="Which pool?",
        ...
        self.assertFalse(cm.exception.question.allow_other)

    def test_seeding_cannot_be_proposed_once_live(self) -> None:
        """The apply path refuses it, so the proposal must never be created."""
        self.tournament.status = Tournament.Status.LIVE
        self.tournament.save(update_fields=["status"])
        before = AgentProposal.objects.count()

        result = propose_update_seeding(self.ctx, seeding={"1": 2, "2": 1})

        self.assertIn("cannot be changed once the tournament has started", result["error"])
        self.assertEqual(AgentProposal.objects.count(), before)
...
class SnakeAndBracketConventionTests(TestCase):
    ...
    def test_one_four_bracket_includes_third_place(self) -> None:
        tournament = Tournament.objects.create(event=create_event(title="Bracket plan"))
        tournament.teams.add(
            *[
                Team.objects.create(name=f"Bracket team {i}", ultimate_central_id=900 + i)
                for i in range(4)
            ]
        )
        plan = bracket_match_plan(tournament, "1-4")
...
class PhasePolicyTests(TestCase):
    ...
    def test_menu_matches_the_phase(self) -> None:
        ...
        # update_tournament_seeding refuses once live, so the tool must not be
        # offered here: it could only ever produce a card that fails on Confirm.
        self.assertNotIn("propose_update_seeding", tools_for(Phase.LIVE))
