...
def propose_create_bracket(ctx: ToolContext, name: str, sequence_number: int) -> dict[str, Any]:
    try:
        matches = bracket_match_plan(ctx.tournament, name)
...
def propose_update_seeding(ctx: ToolContext, seeding: dict[str, int]) -> dict[str, Any]:
    # Re-seeding rewrites every pool and Swiss snapshot, which is only meaningful
    # while matches still hold placeholder seeds. The apply path refuses after
    # that, so refuse here too rather than leaving staff a Confirm that cannot work.
    if ctx.tournament.status in (Tournament.Status.LIVE, Tournament.Status.COMPLETED):
        message = (
            "Seeding cannot be changed once the tournament has started. "
            "Fix an individual match with propose_update_match instead."
        )
        return {"error": message, "message": message}
    return ctx.create_proposal(
