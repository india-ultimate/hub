    ...
    def import_eligible_voters(self, csv_data: str) -> None:
        ...
        for row in reader:
            ...
            try:
                ...
                EligibleVoter.objects.create(election=self, user=user)
...
class VoterVerification(models.Model):
    ...
    def generate_token(cls, election_id: int, user_id: int) -> str:
        ...
        cls.objects.create(election_id=election_id, user_id=user_id, verification_token=token)
        return token
...
class ElectionResult(models.Model):
    ...
    status = models.CharField(
        max_length=20,
        choices=[("active", "Active"), ("eliminated", "Eliminated"), ("winner", "Winner")],
    )
    created_at = models.DateTimeField(auto_now_add=True)
